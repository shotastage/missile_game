...
export class Board {
  ...
  reset() {
    this.board = Array(9)
      .fill()
      .map(() => Array(9).fill({ type: PieceType.EMPTY, isPlayer: true }));
    this.capturedPieces = {
      player: [],
      opponent: [],
    };
  ...
  // 初期配置を設定
  // board.jsのinitializeBoard関数を修正
  initializeBoard() {
    // 歩兵の配置（プレイヤーは下側）
    for (let col = 0; col < 9; col++) {
      this.setPiece(2, col, { type: PieceType.PAWN, isPlayer: false }); // 相手の歩
      this.setPiece(6, col, { type: PieceType.PAWN, isPlayer: true }); // 自分の歩
    }

    // 香車の配置
    this.setPiece(0, 0, { type: PieceType.LANCE, isPlayer: false });
    this.setPiece(0, 8, { type: PieceType.LANCE, isPlayer: false });
    this.setPiece(8, 0, { type: PieceType.LANCE, isPlayer: true });
    this.setPiece(8, 8, { type: PieceType.LANCE, isPlayer: true });

    // 桂馬の配置
    this.setPiece(0, 1, { type: PieceType.KNIGHT, isPlayer: false });
    this.setPiece(0, 7, { type: PieceType.KNIGHT, isPlayer: false });
    this.setPiece(8, 1, { type: PieceType.KNIGHT, isPlayer: true });
    this.setPiece(8, 7, { type: PieceType.KNIGHT, isPlayer: true });

    // 銀将の配置
    this.setPiece(0, 2, { type: PieceType.SILVER, isPlayer: false });
    this.setPiece(0, 6, { type: PieceType.SILVER, isPlayer: false });
    this.setPiece(8, 2, { type: PieceType.SILVER, isPlayer: true });
    this.setPiece(8, 6, { type: PieceType.SILVER, isPlayer: true });

    // 金将の配置
    this.setPiece(0, 3, { type: PieceType.GOLD, isPlayer: false });
    this.setPiece(0, 5, { type: PieceType.GOLD, isPlayer: false });
    this.setPiece(8, 3, { type: PieceType.GOLD, isPlayer: true });
    this.setPiece(8, 5, { type: PieceType.GOLD, isPlayer: true });

    // 王将・玉将の配置
    this.setPiece(0, 4, { type: PieceType.KING, isPlayer: false }); // 相手の玉将
    this.setPiece(8, 4, { type: PieceType.KING, isPlayer: true }); // 自分の王将

    // 角行の配置
    this.setPiece(1, 1, { type: PieceType.BISHOP, isPlayer: false });
    this.setPiece(7, 7, { type: PieceType.BISHOP, isPlayer: true });

    // 飛車の配置
    this.setPiece(1, 7, { type: PieceType.ROOK, isPlayer: false });
    this.setPiece(7, 1, { type: PieceType.ROOK, isPlayer: true });
  }
  ...
  movePiece(fromRow, fromCol, toRow, toCol, promote = false) {
    ...
    if (targetPiece.type !== PieceType.EMPTY) {
      ...
      this.capturedPieces[piece.isPlayer ? "player" : "opponent"].push(
        capturedPiece
      );
    }
  ...
  dropPiece(row, col, piece) {
    ...
    const capturedPieces = piece.isPlayer
  ...
  getValidDropPositions(piece) {
    ...
    for (let row = 0; row < 9; row++) {
      for (let col = 0; col < 9; col++) {
        if (
          this.getPiece(row, col).type === PieceType.EMPTY &&
  ...
  isValidDrop(row, col, piece) {
    ...
    if (piece.type === PieceType.PAWN) {
      for (let i = 0; i < 9; i++) {
        if (
          this.getPiece(i, col).type === PieceType.PAWN &&
  ...
  canPromote(fromRow, fromCol, toRow, toCol) {
    ...
    return (
      (piece.isPlayer && (fromRow < 3 || toRow < 3)) ||
  ...
  mustPromote(fromRow, fromCol, toRow) {
    ...
    if (piece.type === PieceType.PAWN || piece.type === PieceType.LANCE) {
      return (
        (piece.isPlayer && toRow === 0) || (!piece.isPlayer && toRow === 8)
      );
    }
  ...
  isCheckmate(isPlayerKing) {
    ...
    // 持ち駒による回避
    const capturedPieces = isPlayerKing
      ? this.capturedPieces.player
      : this.capturedPieces.opponent;
    for (const piece of capturedPieces) {
      const positions = this.getValidDropPositions({
        ...piece,
        isPlayer: isPlayerKing,
      });
